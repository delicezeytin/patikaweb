...
router.put('/', authMiddleware, async (req, res) => {
    try {
        const { emailjsServiceId, emailjsTemplateId, emailjsPublicKey, calendarId, googleServiceAccountJson, smtpHost, smtpPort, smtpUser, smtpPass, notificationEmail, timezone } = req.body;

        const settings = await prisma.systemSettings.upsert({
            where: { id: 1 },
            update: { emailjsServiceId, emailjsTemplateId, emailjsPublicKey, calendarId, googleServiceAccountJson, smtpHost, smtpPort, smtpUser, smtpPass, notificationEmail, timezone },
            create: { id: 1, emailjsServiceId, emailjsTemplateId, emailjsPublicKey, calendarId, googleServiceAccountJson, smtpHost, smtpPort, smtpUser, smtpPass, notificationEmail, timezone: timezone || 'Europe/Istanbul' }
        });
